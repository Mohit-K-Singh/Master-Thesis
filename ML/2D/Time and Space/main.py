...
from energy import variational_loss, initial_loss, boundary_loss, phase_evolution_loss
...
def main():
    ...
    x_phase = torch.linspace(-3,3,25).to(device)
    t_phase = torch.linspace(0, 1, 25).to(device)
    epoch = 0
    best_loss = float('inf')
    best_l2 = float('inf')
    ...
    for epoch in tqdm(range(epoch+1, n_epochs)):
        ...
        optimizer.zero_grad()
        x_interior,t_interior, x_initial, t_initial, x_boundary, t_boundary = sample_space_time_points(device)
        #psi_init = psi_true(x_initial,t_zero)
        ...
        total_loss = loss_initial   +  loss_interior +  loss_conservation  + loss_phase_evo   +loss_boundary + loss_neg
        ...
        loss_history['L2 ERROR'].append(l2_error.cpu())
