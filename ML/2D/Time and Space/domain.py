...
def sample_space_time_points(device,n_interior=40, n_boundary=500, n_initial=40, x_range=(-4.0, 4.0), t_range=(0.0, 1)):
    a, b = x_range
    t0, t1 = t_range
    ...
    #t_initial = torch.zeros_like(x_initial[:,0], device=device)

    x_initial = torch.linspace(-4,4, n_initial).to(device)
    t_initial = torch.zeros_like(x_initial).to(device)

    lin,_ = torch.sort(torch.rand(n_boundary*4, 1, device=device) * (b-a) + a, dim=0)
