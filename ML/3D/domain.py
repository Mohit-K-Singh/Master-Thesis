...
def sample_points(n_interior=10000, n_boundary=700):
    a, b = -2.5, 2.5
    ...
    lin = torch.rand(n_boundary * 6, 2) * (b - a) + a
    ...
    back = torch.cat([torch.full((n_boundary, 1), a), lin[2*n_boundary:3*n_boundary]], dim=1)
    front = torch.cat([torch.full((n_boundary, 1), b), lin[3*n_boundary:4*n_boundary]], dim=1)

    left = torch.stack([
        lin[4*n_boundary:5*n_boundary, 0],
        torch.full((n_boundary,), a),
        lin[4*n_boundary:5*n_boundary, 1]
    ], dim=1)

    right = torch.stack([
        lin[5*n_boundary:, 0],
        torch.full((n_boundary,), b),
        lin[5*n_boundary:, 1]
    ], dim=1)
