...
from l2_error import compute_l2_error, compute_l2_h1_error
"""
...
def true_psi_3d(xx, yy, zz):
    """
    Returns the 3d groundstate soltuion
    """
    return (1 / np.pi**(3/4)) * np.exp(-0.5 * (xx**2 + yy**2 + zz**2))

def grad_true_psi_3d(xx, yy, zz):
    psi = (1 / np.pi**(3/4)) * np.exp(-0.5 * (xx**2 + yy**2 + zz**2))
    grad_x = -xx * psi
    grad_y = -yy * psi
    grad_z = -zz * psi
...
def main():
    ...
    n_epochs = 1000 # tried 50k steps. Not much change
    ...
    dx = 2 * L / (n_points - 1)
    volume_element = dx **3
    psi_true = true_psi_3d(xx, yy, zz).cpu().numpy()
    grad_psi_true = grad_true_psi_3d(xx, yy, zz)
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        l2_error, h1_error = compute_l2_h1_error(model, grid_points.to(device), n_points, psi_true, grad_psi_true, volume_element)
        l2.append(l2_error)
        h1.append(h1_error)
